"""Personnel / credential verification tables.

Feeds the Personnel Look-Up tool that verifies H-1B beneficiary credentials
(university existence, degree program offering, signatory authenticity,
impossible timelines, duplicate identities, and unaccredited evaluators).
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint(
            "name_normalized", "country", name="uq_university_name_country"
        ),
    )
    op.create_index("idx_university_name_norm", "universities", ["name_normalized"])
    op.create_index(
        "idx_university_accreditation", "universities", ["accreditation_status"]
    )
    ...
    op.create_index(
        "idx_program_name_norm", "university_programs", ["program_name_normalized"]
    )
    op.create_index(
        "idx_program_university", "university_programs", ["university_id"]
    )
    ...
    op.create_index(
        "idx_signatory_name_norm",
        "university_signatories",
        ["name_normalized"],
    )
    op.create_index(
        "idx_signatory_university",
        "university_signatories",
        ["university_id"],
    )
    ...
    op.create_index(
        "idx_evaluator_name_norm", "credential_evaluators", ["name_normalized"]
    )
    ...
    op.create_index(
        "idx_beneficiary_name_norm", "beneficiaries", ["name_normalized"]
    )
    op.create_index(
        "idx_beneficiary_name_dob",
        "beneficiaries",
        ["name_normalized", "date_of_birth"],
    )
    op.create_index(
        "idx_beneficiary_passport",
        "beneficiaries",
        ["passport_country", "passport_last4"],
    )
    ...
    op.create_index(
        "idx_claim_beneficiary", "credential_claims", ["beneficiary_id"]
    )
    ...
    op.create_index(
        "idx_credflag_beneficiary", "credential_flags", ["beneficiary_id"]
    )
    ...
    if bind.dialect.name == "postgresql":
        for table in ("beneficiaries", "credential_claims", "credential_flags"):
            op.execute(
                f"ALTER TABLE {table} ALTER COLUMN raw TYPE JSONB "
                f"USING raw::text::jsonb"
                if table != "credential_flags"
                else f"ALTER TABLE {table} ALTER COLUMN evidence TYPE JSONB "
                f"USING evidence::text::jsonb"
            )
        op.execute(
            "ALTER TABLE universities ALTER COLUMN raw TYPE JSONB "
